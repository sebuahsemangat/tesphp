...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $folder = 'code';
    $filecode = "codesiswa_" . $_SESSION['siswa'] . "_" . $id_soal . ".js";
    ...
    $s_test_file = "/s_testcode_" . $_SESSION['siswa'] . "_" . $id_soal . "_" . $data_s_testcase["id_s_testcase"] . ".js";
    $s_test_path = $folder . $s_test_file;

    // Membuat file s_test_code dan menulis kode di dalamnya
    $s_test_code = '
const ' . $soal["function_name"] . " = require('./" . str_replace(".js","",$filecode) . "')" .
'
'.
'console.log('. $soal["function_name"] . '(' . $data_s_testcase["input"] .'));';
    file_put_contents($s_test_path, $s_test_code);

    // Menyiapkan perintah untuk menjalankan script dengan batasan waktu
    $command = "node " . escapeshellarg($s_test_path);
    $descriptorspec = [
        0 => ["pipe", "r"], // stdin
        1 => ["pipe", "w"], // stdout
        2 => ["pipe", "w"]  // stderr
    ...
    if (trim($output) != $data_s_testcase["output"] || !empty($error)) { //jika ada error maka tampilkan pesan errornya
        ...
        while ($data_testcase = mysqli_fetch_assoc($select_testcase)) {
            ...
            $test_file = "/testcode_" . $_SESSION['siswa'] . "_" . $id_soal . "_" . $data_testcase["id_testcase"] . ".js";
...
'
'.
'console.log('. $soal["function_name"] . '(' . $data_testcase["input"] .'));';
            file_put_contents($test_path, $test_code);

            // Menyiapkan perintah untuk menjalankan script dengan batasan waktu
            $command = "node " . escapeshellarg($test_path);
            $descriptorspec = [
                0 => ["pipe", "r"], // stdin
                1 => ["pipe", "w"], // stdout
                2 => ["pipe", "w"]  // stderr
